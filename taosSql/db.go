...
func getError() error {
	if errno := getErrno(); errno != 0 {
		return &errors.TaosError{Code: errno & 0xffff, ErrStr: tstrerror(errno)}
	}
	return nil
}
...
func (db *taosDB) Subscribe(restart bool, name string, sql string, interval time.Duration) (topic Topic, err error) {
	...
	if topic == nil {
		...
		if err != nil {
			...
			err = &errors.TaosError{Code: 0xffff, ErrStr: "failed to subscribe"}
			return
		}
	}
	return topic, nil
...
func (db *taosDB) execute(sql string, params []driver.Value) (res *taosRes, err error) {
	...
	if rc := stmt.prepare(sql); rc != 0 {
		err = &errors.TaosError{Code: rc & 0xffff, ErrStr: tstrerror(rc)}
		return
	}
...
func (db *taosDB) Exec(sql string, params ...driver.Value) (result driver.Result, err error) {
	...
	if res == nil {
		if err = getError(); err == nil {
			err = &errors.TaosError{Code: 0xffff, ErrStr: fmt.Sprintf("failed to exec: %s", sql)}
		}
		return
	}
	defer res.freeResult()
	if errno := res.errno(); errno != 0 {
		err = &errors.TaosError{Code: errno & 0xffff, ErrStr: res.errstr()}
		return
	}
...
func (db *taosDB) Query(sql string, params ...driver.Value) (rows driver.Rows, err error) {
	...
	if res == nil {
		if err = getError(); err == nil {
			err = &errors.TaosError{Code: 0xffff, ErrStr: "failed to query"}
		}
		return
	}
	errno := res.errno()
