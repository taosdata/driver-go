...
func main() {
	...
	_, err = db.Exec("CREATE DATABASE IF NOT EXISTS example_stmt_geometry_ws")
	if err != nil {
		log.Fatalln("Failed to create database example_stmt_geometry_ws, ErrMessage: " + err.Error())
	}
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS example_stmt_geometry_ws.ntb (ts TIMESTAMP, val GEOMETRY(100))")
	if err != nil {
		log.Fatalln("Failed to create table example_stmt_geometry_ws.ntb, ErrMessage: " + err.Error())
	}

	config := stmt.NewConfig(fmt.Sprintf("ws://%s:6041", host), 0)
	config.SetConnectUser("root")
	config.SetConnectPass("taosdata")
	config.SetConnectDB("example_stmt_geometry_ws")
	...
	columnType := param.NewColumnType(2).AddTimestamp().AddGeometry(100)
	...
	columnData[1] = param.NewParam(1).AddGeometry(geometryData)
	...
	fmt.Printf("Successfully inserted %d rows to example_stmt_geometry_ws.ntb.\n", affected)
