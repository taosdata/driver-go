...
func main() {
	...
	_, err = db.Exec("CREATE DATABASE IF NOT EXISTS example_stmt_varbinary_native")
	if err != nil {
		log.Fatalln("Failed to create database example_stmt_varbinary_native, ErrMessage: " + err.Error())
	}
	_, err = db.Exec("USE example_stmt_varbinary_native")
	if err != nil {
		log.Fatalln("Failed to use database example_stmt_varbinary_native, ErrMessage: " + err.Error())
	}
	_, err = db.Exec("CREATE TABLE IF NOT EXISTS ntb (ts TIMESTAMP, val VARBINARY(100))")
	if err != nil {
	...
	// select * from example_stmt_varbinary_native.ntb
}
